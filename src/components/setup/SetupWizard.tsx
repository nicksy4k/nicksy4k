...
import { useCycleSettings, getActiveCycle, type CycleType, type CycleSettings } from "@/lib/cycle";
import {
  useCategories,
  useCommitments,
  useIncomes,
...
export function SetupWizard() {
  ...
  const [catDraft, setCatDraft] = useState<string[]>(
    expenseCategories.length ? expenseCategories : [],
  );
  ...
  const stepMeta = [
    { title: "Cycle settings", icon: CalendarClock, desc: "How Ledgerly slices your month." },
    {
      ...
      icon: Wallet,
      desc: "Your starting money and any savings buckets.",
    },
          ...
          {step === 0 && (
            <div className="space-y-4">
              ...
              <div className="flex items-start justify-between gap-3 rounded-lg border border-border p-4">
                <div>
                  <Label htmlFor="wiz-carry" className="text-sm">
                    Carry unspent balance forward
                  </Label>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    At the end of each cycle, any leftover "Left to spend" becomes a Carryover
                    income entry.
                  </p>
                </div>
              ...
              <div className="rounded-md bg-primary/5 border border-primary/20 px-3 py-2 text-sm">
                ...
                <span className="font-medium tabular-nums">
                  {format(parseISO(previewCycle.startISO), "d MMM yyyy")} –{" "}
                  {format(parseISO(previewCycle.endISO), "d MMM yyyy")}
                </span>
              </div>
            </div>
          )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {pockets.map((p, i) => (
                        ...
                        >
                          <Input
                            placeholder="Pocket name (e.g. Bills, Savings)"
                            value={p.name}
                            onChange={(e) =>
                              setPockets((prev) =>
                          ...
                          <div className="relative">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setPockets((prev) =>
                                  prev.map((x, j) =>
                                    j === i ? { ...x, startingAmount: e.target.value } : x,
                                  ),
                                )
                              }
                            />
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => setPockets((prev) => prev.filter((_, j) => j !== i))}
                          >
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        setPockets((prev) => [...prev, { id: uid(), name: "", startingAmount: "" }])
                      }
                    >
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {catDraft.length === 0 && (
                      <span className="text-xs text-muted-foreground">
                        Defaults will be seeded on finish.
                      </span>
                    )}
                  </div>
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter" && newCat.trim()) {
                          e.preventDefault();
                          if (!catDraft.includes(newCat.trim()))
                            setCatDraft((p) => [...p, newCat.trim()]);
                          setNewCat("");
                        }
                      }}
                    />
                ...
                <div className="space-y-5">
                  <div className="space-y-2">
                    ...
                    {recurringDrafts.map((r, i) => (
                      <div
                        key={r.id}
                        className="grid grid-cols-[1fr_120px_140px_140px_auto] gap-2 items-center"
                      >
                        <Input
                          placeholder="Source (e.g. Salary)"
                          value={r.source}
                          onChange={(e) =>
                            setRecurringDrafts((p) =>
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            setRecurringDrafts((p) =>
                              p.map((x, j) =>
                                j === i ? { ...x, cadence: v as IncomeCadence } : x,
                              ),
                            )
                          }
                        >
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setRecurringDrafts((p) =>
                              p.map((x, j) => (j === i ? { ...x, nextDate: e.target.value } : x)),
                            )
                          }
                        />
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => setRecurringDrafts((p) => p.filter((_, j) => j !== i))}
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
                    ...
                    {commitmentDrafts.map((c, i) => (
                      ...
                      >
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCommitmentDrafts((p) =>
                              p.map((x, j) => (j === i ? { ...x, dueDate: e.target.value } : x)),
                            )
                          }
                        />
                        <Input
                          ...
                          onChange={(e) =>
                            setCommitmentDrafts((p) =>
                              p.map((x, j) => (j === i ? { ...x, category: e.target.value } : x)),
                            )
                          }
                        />
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => setCommitmentDrafts((p) => p.filter((_, j) => j !== i))}
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
...
function ModeToggle({
  ...
  return (
    ...
    >
      <label
        className={`flex items-start gap-3 rounded-lg border p-3 cursor-pointer transition ${mode === "keep" ? "border-primary bg-primary/5" : "border-border"}`}
      >
      ...
      <label
        className={`flex items-start gap-3 rounded-lg border p-3 cursor-pointer transition ${mode === "replace" ? "border-primary bg-primary/5" : "border-border"}`}
      >
