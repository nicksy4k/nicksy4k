...
export function ConnectedAccountsCard() {
  ...
  const primaryEmail =
    (email?.identity_data?.email as string | undefined) ??
      ...
      <CardContent className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {email
                  ? ((email.identity_data?.email as string | undefined) ?? "Connected")
                  : "Not connected"}
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between rounded-lg border border-border/60 bg-card/40 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {google
                  ? ((google.identity_data?.email as string | undefined) ?? "Connected")
                  : "Not connected"}
              </p>
            </div>
          </div>
        ...
        {!google && (
          <div className="rounded-lg border border-border/60 bg-muted/30 p-4 text-xs text-muted-foreground space-y-2">
            ...
            <p>
              If the emails match and both are verified, Google will attach to this same account. If
              you use a different Google email, it will create a separate account instead.
            </p>
          </div>
        )}
      </CardContent>
